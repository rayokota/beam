...
/**
 * Kafka Streams operator for {@link org.apache.beam.sdk.transforms.windowing.Window.Assign}.
 */
public class WindowAssignOp<T, W extends BoundedWindow> implements Op<T, T> {
  private static final Logger LOG = LoggerFactory.getLogger(WindowAssignOp.class);
  ...
  public void processElement(WindowedValue<T> inputElement, OpEmitter<T> emitter) {
    ...
    try {
      windows = windowFn.assignWindows(new KafkaStreamsAssignContext<>(windowFn, inputElement));
    } catch (Exception e) {
    ...
    windows.stream()
        ...
        .forEach(outputElement -> {
          LOG.debug("window emit {}", outputElement);
          emitter.emitElement(outputElement);
        });
  }
}
