...
/**
 * {@link StateInternals} that uses local {@link KeyValueStore} to manage state.
 */
public class KafkaStreamsStoreStateInternals<K> implements StateInternals {
  ...
  private KafkaStreamsStoreStateInternals(
  ...
  public <V extends State> V state(
    ...
    return address.bind(new StateTag.StateBinder() {
      ...
      public <T> ValueState<T> bindValue(
          StateTag<ValueState<T>> spec,
          Coder<T> coder
      ) {
        return new KafkaStreamsValueState<>(namespace, address, coder);
      }

      @Override
      public <T> BagState<T> bindBag(
          StateTag<BagState<T>> spec,
          Coder<T> elemCoder
      ) {
        return new KafkaStreamsBagState<>(namespace, address, elemCoder);
      }

      @Override
      public <T> SetState<T> bindSet(
          StateTag<SetState<T>> spec,
          Coder<T> elemCoder
      ) {
      ...
      bindCombiningValue(
        ...
        return new KafkaStreamsAccumulatorCombiningState<>(namespace, address, accumCoder, combineFn);
      }
      ...
      public WatermarkHoldState bindWatermark(
        ...
        return new KafkaStreamsWatermarkHoldState(namespace, address, timestampCombiner);
      }
    });
  }
  ...
  /**
   * Factory class to create {@link KafkaStreamsStoreStateInternals}.
   */
  public static class Factory<K> implements StateInternalsFactory<K> {
    ...
    public StateInternals stateInternalsForKey(K key) {
      ...
      return new KafkaStreamsStoreStateInternals<>(store, key, baos.toByteArray());
    }
  }

  private abstract class AbstractKafkaStreamsState<T> {
    private final Coder<T> coder;
    private final byte[] encodedStoreKey;
    private final String namespace;
    private final String addressId;

    protected AbstractKafkaStreamsState(
        StateNamespace namespace,
    ...
    public boolean equals(Object o) {
      ...
      @SuppressWarnings("unchecked") final AbstractKafkaStreamsState<?> that = (AbstractKafkaStreamsState<?>) o;
  ...
  private class KafkaStreamsValueState<T> extends AbstractKafkaStreamsState<T> implements ValueState<T> {
    private KafkaStreamsValueState(
        StateNamespace namespace,
        StateTag<? extends State> address,
        Coder<T> coder
    ) {
  ...
  private class KafkaStreamsBagState<T> extends AbstractKafkaStreamsState<T> implements BagState<T> {

    private KafkaStreamsBagState(
        StateNamespace namespace,
        StateTag<? extends State> address,
        Coder<T> coder
    ) {
    ...
    public List<T> read() {
      synchronized (store) {
        ...
        while (iter.hasNext()) {
          list.add(decodeValue(iter.next().value));
        }
        return list;
      }
    }
  ...
  private class KafkaStreamsAccumulatorCombiningState<InT, AccumT, OutT>
      extends AbstractKafkaStreamsState<AccumT>
      implements CombiningState<InT, AccumT, OutT> {

    private final Combine.CombineFn<InT, AccumT, OutT> combineFn;

    protected KafkaStreamsAccumulatorCombiningState(
        StateNamespace namespace,
        StateTag<? extends State> address,
        Coder<AccumT> coder,
  ...
  private class KafkaStreamsWatermarkHoldState
      extends AbstractKafkaStreamsState<Instant>
      implements WatermarkHoldState {

    private final TimestampCombiner timestampCombiner;

    public <V extends State> KafkaStreamsWatermarkHoldState(
        StateNamespace namespace,
        StateTag<V> address,
        TimestampCombiner timestampCombiner
    ) {
