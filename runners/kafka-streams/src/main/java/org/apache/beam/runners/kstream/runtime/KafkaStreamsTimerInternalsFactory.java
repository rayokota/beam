...
import org.apache.beam.runners.kstream.KafkaStreamsRunner;
...
/**
 * {@link TimerInternalsFactory} that creates {@link TimerInternals}.
 ...
 */
public class KafkaStreamsTimerInternalsFactory<K> implements TimerInternalsFactory<K> {
  private static final Logger LOG = LoggerFactory.getLogger(KafkaStreamsTimerInternalsFactory.class);
  ...
  public KafkaStreamsTimerInternalsFactory(Coder<K> keyCoder) {
  ...
  public TimerInternals timerInternalsForKey(K key) {
    ...
    if (keyCoder == null) {
      if (key != null) {
        throw new IllegalArgumentException(
            String.format(
                "Received non-null key for unkeyed timer factory. Key: %s",
                key
            ));
      }
      keyBytes = null;
    ...
    return new KafkaStreamsTimerInternals(keyBytes, key);
  }

  public void setInputWatermark(Instant watermark) {
    if (watermark.isBefore(inputWatermark)) {
      throw new IllegalArgumentException("New input watermark " + inputWatermark +
          " is before current watermark " + watermark);
    }
  ...
  private class KafkaStreamsTimerInternals implements TimerInternals {
    private final byte[] keyBytes;
    private final K key;

    public KafkaStreamsTimerInternals(byte[] keyBytes, K key) {
    ...
    public void setTimer(TimerData timerData) {
      if (!timerData.getDomain().equals(TimeDomain.EVENT_TIME)) {
        throw new UnsupportedOperationException(
            String.format("%s currently only supports even time", KafkaStreamsRunner.class));
      }
    ...
    public void deleteTimer(TimerData timerData) {
      if (!timerData.getDomain().equals(TimeDomain.EVENT_TIME)) {
        throw new UnsupportedOperationException(
            String.format("%s currently only supports event time", KafkaStreamsRunner.class));
      }
    ...
    public Instant currentProcessingTime() {
      throw new UnsupportedOperationException(
          String.format("%s does not currently support processing time", KafkaStreamsRunner.class));
    }
