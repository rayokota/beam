...
/**
 * Kafka Streams operator for {@link org.apache.beam.sdk.transforms.GroupByKey}.
 */
public class GroupByKeyOp<K, V> implements Op<KeyedWorkItem<K, V>, KV<K, Iterable<V>>> {
  ...
  private transient KafkaStreamsTimerInternalsFactory<K> timerInternalsFactory;
  ...
  public void open(
    ...
    @SuppressWarnings("unchecked") final KeyValueStore<byte[], byte[]> store =
    ...
    this.timerInternalsFactory = new KafkaStreamsTimerInternalsFactory<>(inputCoder.getKeyCoder());
  ...
  public void processElement(
      WindowedValue<KeyedWorkItem<K, V>> inputElement,
      OpEmitter<KV<K, Iterable<V>>> emitter
  ) {
  ...
  private void fireTimer(K key, TimerInternals.TimerData timer) {
    ...
    WindowedValue<KeyedWorkItem<K, V>> element = WindowedValue.valueInGlobalWindow(
