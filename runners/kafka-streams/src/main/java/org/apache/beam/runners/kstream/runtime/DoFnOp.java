...
/**
 * Kafka Streams operator for {@link DoFn}.
 */
public class DoFnOp<InT, FnOutT, OutT> implements Op<InT, OutT> {
  ...
  private transient KafkaStreamsTimerInternalsFactory<?> timerInternalsFactory;
  ...
  @Override
  public void open(ProcessorContext context, OpEmitter<OutT> emitter) {
    this.inputWatermark = BoundedWindow.TIMESTAMP_MIN_VALUE;
    this.pushbackWatermarkHold = BoundedWindow.TIMESTAMP_MAX_VALUE;

    this.timerInternalsFactory = new KafkaStreamsTimerInternalsFactory<>(null);

    final KeyValueStore<byte[], byte[]> store =
        (KeyValueStore<byte[], byte[]>) context.getStateStore("beamStore");

    this.stateInternalsFactory =
        new KafkaStreamsStoreStateInternals.Factory<>(
            mainOutputTag.getId(),
            store,
            VoidCoder.of()
        );
    ...
    this.fnRunner = DoFnRunners.simpleRunner(
        pipelineOptions,
        doFn,
        sideInputHandler,
  ...
  public void processElement(WindowedValue<InT> inputElement, OpEmitter<OutT> emitter) {
    assert !(inputElement instanceof ControlMessage);
  ...
  public void processSideInput(
    ...
    @SuppressWarnings("unchecked") final WindowedValue<Iterable<?>> retypedElements = (WindowedValue<Iterable<?>>) sideInput;
  ...
  private void emitAllPushbackValues() {
    if (!pushbackValues.isEmpty()) {
      ...
      pushbackWatermarkHold = BoundedWindow.TIMESTAMP_MAX_VALUE;
      pushbackValues.clear();
      previousPushbackValues.forEach(value -> {
    ...
    public DoFnRunners.OutputManager create(OpEmitter<OutT> emitter) {
      return new DoFnRunners.OutputManager() {
        ...
        public <T> void output(TupleTag<T> tupleTag, WindowedValue<T> windowedValue) {
          ...
          @SuppressWarnings("unchecked") final WindowedValue<OutT> retypedWindowedValue = (WindowedValue<OutT>) windowedValue;
          LOG.debug("single emit {}", retypedWindowedValue);
    ...
    public DoFnRunners.OutputManager create(OpEmitter<RawUnionValue> emitter) {
      return new DoFnRunners.OutputManager() {
        ...
        public <T> void output(TupleTag<T> tupleTag, WindowedValue<T> windowedValue) {
          ...
          WindowedValue<RawUnionValue> value = windowedValue.withValue(rawUnionValue);
          LOG.debug("multi emit {}", value);
          emitter.emitElement(value);
        }
      };
    }
  }
