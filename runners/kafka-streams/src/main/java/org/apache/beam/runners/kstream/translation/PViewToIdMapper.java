...
public class PViewToIdMapper extends Pipeline.PipelineVisitor.Defaults {
  ...
  public void visitValue(PValue value, TransformHierarchy.Node producer) {
    ...
    final String kafkaStreamsSafeValueDesc = valueDesc.replaceAll("[^A-Za-z0-9_-]", "_");

    idMap.put(value, String.format("%d-%s", nextId++, kafkaStreamsSafeValueDesc));
  }

  @Override
  public void visitPrimitiveTransform(TransformHierarchy.Node node) {
    if (node.getTransform() instanceof KafkaStreamsPublishView) {
      final PCollectionView view = ((KafkaStreamsPublishView) node.getTransform()).getView();
